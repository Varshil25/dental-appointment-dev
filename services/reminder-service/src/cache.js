...
redis?.on('error', (err) => console.error('[reminder-service] redis error:', err.message));

const PREFIX = 'reminder-service:';
...
export async function cached(key, ttlSeconds, fetchFn) {
  ...
  try {
    ...
    console.error('[reminder-service] cache read failed:', err.message);
  }
  const value = await fetchFn();
  try {
    await redis.set(fullKey, JSON.stringify(value), 'EX', ttlSeconds);
  } catch (err) {
    console.error('[reminder-service] cache write failed:', err.message);
  }
  return value;
}
...
export async function invalidate(pattern) {
  ...
  try {
    ...
    console.error('[reminder-service] cache invalidate failed:', err.message);
  }
}

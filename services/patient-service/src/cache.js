...
redis?.on('error', (err) => console.error('[patient-service] redis error:', err.message));

const PREFIX = 'patient-service:';
...
export async function cached(key, ttlSeconds, fetchFn) {
  ...
  try {
    ...
    console.error('[patient-service] cache read failed:', err.message);
  }
  const value = await fetchFn();
  try {
    await redis.set(fullKey, JSON.stringify(value), 'EX', ttlSeconds);
  } catch (err) {
    console.error('[patient-service] cache write failed:', err.message);
  }
  return value;
}
...
export async function invalidate(pattern) {
  ...
  try {
    ...
    console.error('[patient-service] cache invalidate failed:', err.message);
  }
}

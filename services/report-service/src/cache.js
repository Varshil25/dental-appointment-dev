...
redis?.on('error', (err) => console.error('[report-service] redis error:', err.message));

const PREFIX = 'report-service:';

// Read-through cache: hit -> parsed JSON in ~1ms; miss -> call fetchFn(),
// cache the result with an EX ttl, return it.
export async function cached(key, ttlSeconds, fetchFn) {
  ...
  try {
    ...
    console.error('[report-service] cache read failed:', err.message);
  }
  const value = await fetchFn();
  try {
    await redis.set(fullKey, JSON.stringify(value), 'EX', ttlSeconds);
  } catch (err) {
    console.error('[report-service] cache write failed:', err.message);
  }
  return value;
}
...
export async function invalidate(pattern) {
  ...
  try {
    ...
    console.error('[report-service] cache invalidate failed:', err.message);
  }
}
